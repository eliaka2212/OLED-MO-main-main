...
class QuestionApi {
  ...
  Future<List<QuestionAvecReponses>> getQuestion(
    int nbQuestion,
    String categorieQuestion,
  ) async {
    final response = await http.get(
      Uri.parse(
        '${AppConfig.apiBaseUrl}/api/questions/$nbQuestion?categorie=$categorieQuestion',
      ),
    );
    if (response.statusCode == 200) {
      ...
      return listeQuestions
          .map((q) => QuestionAvecReponses.fromJson(q))
          .toList();
    } else {
